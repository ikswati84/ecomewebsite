<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden group h-full flex flex-col hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
    <!-- Product Image -->
    <div class="relative aspect-square overflow-hidden bg-gray-200 dark:bg-gray-700">
      ...
      <!-- Quick Actions -->
      <div class="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex space-x-2">
        <router-link
          :to="`/product/${product.id}`"
          @click="addToRecentProducts"
        ...
        <button 
          ...
          :class="isInWishlist ? 'text-red-500' : 'text-gray-600 dark:text-gray-400'"
    ...
    <div class="p-5 flex-1 flex flex-col">
      ...
      <div v-if="product.tags && product.tags.length > 0" class="flex flex-wrap gap-1 mb-3">
        ...
        <span v-if="product.tags.length > 2" class="text-xs text-gray-500 dark:text-gray-500">
      ...
      <button
        ...
        @click="addToCart"
        class="w-full bg-primary-600 text-white py-3 px-4 rounded-lg font-semibold hover:bg-primary-700 transition-colors flex items-center justify-center space-x-2 mt-auto disabled:opacity-50"
        :disabled="isLoading"
      >
        <ShoppingCartIcon v-if="!isLoading" class="h-4 w-4" />
      ...
      <router-link
        v-else-if="isInCart"
        to="/checkout"
        class="w-full bg-primary-600 text-white py-3 px-4 rounded-lg font-semibold hover:bg-primary-700 transition-colors flex items-center justify-center space-x-2 mt-auto"
      >
        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
      ...
      <button
        v-else
        disabled
        class="w-full bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 py-3 px-4 rounded-lg font-semibold cursor-not-allowed mt-auto"
      >
        Out of Stock
      </button>
    </div>
